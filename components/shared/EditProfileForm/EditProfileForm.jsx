...
class EditProfileForm extends Component {
  ...
  handlePrivacyChange = (evt) => {
    const { form } = this.state;
    const field = form.fields.isPrivate;
    const updatedField = {
      ...field,
  ...
  showError = () => {
    const { snackError } = this.state;
    if (!snackError.visible) {
      this.setState(prevState => ({
        ...prevState,
        snackError: {
  ...
  render() {
    ...
    const {
      form: {
        ...
        hasChanged,
        fields: {
          name, displayName, isPrivate, country, favoriteTeam,
        },
      },
    } = this.state;
    ...
    return (
      <Mutation mutation={UPDATE_USER_PROFILE_MUTATION} variables={variables}>
        {(updateUserProfile, { error, loading }) => (
          <div className={classNames(classes.root)}>
            {error && <ErrorMessage message="Error updated profile" />}
            <form
              method="POST"
              onSubmit={async (e) => {
                e.preventDefault();
